...
- Landing (`js/index-landing.js`) réutilise le setup `--app-vh`, installe un dots-nav par IntersectionObserver et clone le branding fixe. L’animation de fond (`js/utility/animated-bg.js`) s’active uniquement via `body.landing`, respecte `prefers-reduced-motion` et recalcule sur resize.
...
- Les assets `img/competences/star-*.png` sont absents ; ne pas utiliser les classes associées tant que les fichiers ne sont pas fournis.

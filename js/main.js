...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (homeLink) {
    homeLink.addEventListener("click", function () {
      ...
      const isCvPage =
        currentPath.includes("cv.html") || currentPath.endsWith("/cv.html");
  ...
  const downloadOverlayCloseBtn = downloadOverlay
    ? downloadOverlay.querySelector(".download-overlay-close")
    : null;
  const downloadOverlayPrimaryAction = downloadOverlay
    ? downloadOverlay.querySelector("[data-download-primary]")
    : null;
  let downloadOverlayLastFocus = null;

  function isDownloadOverlayOpen() {
    return (
      downloadOverlay && downloadOverlay.getAttribute("aria-hidden") === "false"
    );
  }

  function focusFirstDownloadControl() {
    if (!downloadOverlay) return;
    const focusable = downloadOverlay.querySelectorAll(
  ...
  function closeDownloadOverlay() {
    ...
    if (
      downloadOverlayLastFocus &&
      typeof downloadOverlayLastFocus.focus === "function"
    ) {
  ...
  const competencesData = Array.isArray(window.CV_COMPETENCES)
    ? window.CV_COMPETENCES
    : [];
  const competencesByPeriode = competencesData.reduce(
  ...
  function renderCompetencesInContainer(container, periode) {
    ...
    const isStaticSection =
      !isDynamicPanel && container.classList.contains("competences-list");
    const shouldBeInteractive =
      isDynamicPanel || (isStaticSection && desktopMediaQuery.matches);
    const listElement = buildCompetencesList(competences, {
  ...
  function populateStaticCompetenceSections() {
    const staticContainers = document.querySelectorAll(
      ".competences-section .competences-list",
    );
  ...
  function attachCompetenceItemActions(listElement) {
    ...
    items.forEach((item) => {
      ...
      const fallbackName =
        item.dataset.competenceName || item.textContent || "";
  ...
  const periodeToRangeValue = sectionsOrder.reduce(
  ...
  function updateContent() {
    ...
    const exactPosition =
      -(currentIndex + sectionProgress) * (100 / sectionCount);
  ...
  function updateStyling(index) {
    ...
    if (
      activeSection &&
      activeSection.id === "trading" &&
      typeof window.resetTradingPressHint === "function"
    ) {
      try {
        window.resetTradingPressHint();
      } catch (_) {}
    }
  }
  ...
  function addSnapMarkers() {
    ...
    function setAllMarkersOpacity(opacity) {
      markerElements.forEach((m) => {
        m.element.style.opacity = opacity;
      });
    }
  ...
  (function addTradingSwipeStrip() {
    ...
    strip.setAttribute(
      "aria-label",
      "Balayez horizontalement pour changer de section",
    );
    strip.innerHTML =
      '<div class="swipe-hint" aria-hidden="true"><span class="arrow left">◀</span><span class="dots"></span><span class="arrow right">▶</span></div>';
  ...
  rangeSlider.addEventListener("touchstart", () => hideSnapMarkers(), {
    passive: true,
  });
  ...
  slider.addEventListener(
    ...
    (e) => {
      ...
      blockSwipe = !!(
        ...
        e.target &&
        e.target.closest &&
        e.target.closest("#trading-live-chart")
      );
  ...
  slider.addEventListener(
    ...
    (e) => {
      ...
      const maxAllowed = Math.min(
        parseFloat(rangeSlider.max),
        (startIndex + 1) * 50,
      );
      const tentative = Math.min(
        maxAllowed,
        Math.max(minAllowed, startIndex * 50 + deltaRange),
      );
  ...
  function navigateTimelineToPeriode(periode) {
    ...
    const minValue = Number.isNaN(parseFloat(rangeSlider.min))
      ? 0
      : parseFloat(rangeSlider.min);
    const maxValue = Number.isNaN(parseFloat(rangeSlider.max))
      ? targetValue
      : parseFloat(rangeSlider.max);

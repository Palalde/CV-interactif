...
addEventListener("DOMContentLoaded", async () => {
  ...
  statsWorker.onmessage = async function (event) {
    ...
    periodFilterContainer.addEventListener("click", async (event) => {
      ...
      if (period === "all") {
        ...
        filteredCompetences = window.CV_COMPETENCES.filter(
          (comp) => comp.periode === period,
        );
      }
  ...
  compareBtn.addEventListener("click", async () => {
    ...
    resultsContainer.innerHTML =
      '<div class="compare-loading">Récupération des données...</div>';

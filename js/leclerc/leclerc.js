...
document.addEventListener("DOMContentLoaded", () => {
  ...
  plate.addEventListener("drop", (e) => {
    ...
    if (!payload) {
      ...
      if (dragging)
        payload = {
          name: dragging.dataset.expName,
          text: dragging.dataset.expText,
          emoji: dragging.querySelector(".fruit-emoji")?.textContent,
        };
    }
    if (payload && payload.name)
      updateExperience(payload.name, payload.text, payload.emoji);
    highlightPlate(false);
  });
  ...
  const beginGlobalDragGuards = () => {
    ...
    if (contentSlider) {
      contentSlider.addEventListener("touchstart", intercept, {
        capture: true,
        passive: false,
      });
      contentSlider.addEventListener("touchmove", intercept, {
        capture: true,
        passive: false,
      });
      contentSlider.addEventListener("touchend", intercept, {
        capture: true,
        passive: false,
      });
    }
    activeDragGuardsCleanup = () => {
      ...
      if (contentSlider) {
        contentSlider.removeEventListener("touchstart", intercept, {
          capture: true,
        });
        contentSlider.removeEventListener("touchmove", intercept, {
          capture: true,
        });
        contentSlider.removeEventListener("touchend", intercept, {
          capture: true,
        });
      }

document.addEventListener("DOMContentLoaded", function () {
  ...
  function onDragMove(clientY) {
    ...
    const closedOffsetPx =
      closedOffsetPxCached != null
        ? closedOffsetPxCached
    ...
    if (!wasOpenAtDragStart) {
      ...
      dyn.style.opacity = String(
        Math.min(1, Math.max(0, closingProgress * 1.1)),
      );
  ...
  btn.addEventListener("click", function (e) {
    ...
    const clickedInsideDyn =
      e.target.closest && e.target.closest("#competences-dynamiques");
  ...
  document
    .querySelectorAll(".competences-section .competence-titre")
    .forEach((h3) => {

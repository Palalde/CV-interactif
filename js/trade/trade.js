...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const applyTheme = () => {
    ...
    const bg = isLight
      ? computed.getPropertyValue("--background-moyen-light")?.trim() ||
        "#edead7"
      : computed.getPropertyValue("--background-moyen-dark")?.trim() ||
        "#20203b";
    const text = isLight
    ...
    if (
      ...
      if (isCoarsePointer) {
        if (!swipeHintActive) {
          ...
          lastTooltipTimeForSwipe &&
          String(dateStr) !== String(lastTooltipTimeForSwipe)
        ) {
          dismissSwipeHint();
  ...
  tooltipThemeObserver.observe(document.body, {
    attributes: true,
  ...
  chart.subscribeCrosshairMove((param) => {
    ...
    const phase = tradingPhaseTextConfig.find((p) =>
      inRange(month, p.start, p.end),
    );
